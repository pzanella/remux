...
/// Byte ranges `(code_start, content_start)` of every start code in
/// `data`, in order — `code_start` is where the start code itself truly
/// begins, `content_start` is wherever the NALU payload after it starts.
///
/// A 3-byte `00 00 01` suffix match alone can't tell a genuine 4-byte code
/// apart from a 3-byte one whose preceding NALU happens to end in its own
/// zero byte (H.264's trailing_zero_8bits padding does this in practice —
/// confirmed against a real libx264-encoded stream) — the two are
/// byte-for-byte identical, an unavoidable ambiguity in Annex-B itself,
/// not something extra lookahead/lookback can fully resolve. This
/// resolves it the same way real decoders do (e.g. FFmpeg's own Annex-B
/// scanner): greedily walk `code_start` back over *every* consecutive
/// leading zero, not just one. A real encoder's own RBSP padding is at
/// most one zero byte, so this reads a real stream exactly as intended;
/// the only case it can't distinguish (a NALU deliberately ending in 2+
/// zero bytes right before a start code) isn't one real encoder output
/// produces.
...
/// Splits one Annex-B encoded access unit (as WebCodecs' `avc: { format:
/// 'annexb' }` output already is) into its SPS/PPS (present on a keyframe,
/// `None` otherwise) and its remaining NALUs reframed as length-prefixed
/// AVCC — the form this project's fMP4 `mdat` wants, and already what a
/// NALU read from a real MP4 file looks like (`avcc_to_annexb` above is
/// this same conversion in the other direction). The Access Unit
/// Delimiter (type 9), if present, is dropped along with SPS/PPS — AVCC
/// samples don't carry one; SEI (type 6) and the actual slice NALU(s) stay
/// in `sample_avcc`, exactly as they're allowed to inside a real AVCC
/// sample.
struct AnnexbNalus {
    sps: Option<Vec<u8>>,
    pps: Option<Vec<u8>>,
...
fn split_annexb(data: &[u8]) -> AnnexbNalus {
    ...
    for (i, &(_, content_start)) in starts.iter().enumerate() {
        // The *next* code's own start (not content_start) is this NALU's
        // end — using content_start here would run one byte past a 4-byte
        // code's own leading zero, back into what looks like this NALU's
        // trailing content but is actually already part of the next code.
        ...
        match nalu[0] & 0x1F {
            ...
            9 => {} // Access Unit Delimiter -- not carried in an AVCC sample.
            _ => {
...
/// Builds an `avcC` box payload from one SPS + one PPS NALU (each without a
/// start code) — the inverse of `parse_avcc_header`. Needed for a
/// WebCodecs-encoded ABR rendition's fMP4 init segment specifically: unlike
/// the fast path, there's no source-file avcC to reuse here, since a
/// re-encoded rendition's own profile/level/dimensions never match the
/// source's. 4-byte NALU lengths (`0xFF`) to match the length size this
/// project already uses everywhere else it writes AVCC (see `mux_segment`'s
/// `nalu_len_size` handling).
fn build_avcc(sps: &[u8], pps: &[u8]) -> Vec<u8> {
    ...
    out.push(0xFF); // reserved(6)=111111 | lengthSizeMinusOne(2)=11 (4-byte lengths)
    out.push(0xE1); // reserved(3)=111 | numOfSequenceParameterSets(5)=1
...
/// A `TrackData` carrying only the two or three fields fMP4's init-segment
/// writer actually reads (`timescale`, `avcc_raw`, `aac_config`) — used for
/// a WebCodecs-encoded ABR rendition, which has no real parsed track to
/// build one from. Every other field is a value that's simply never read
/// for this purpose (see `fmp4::video_trak`/`audio_trak`).
...
/// Builds `SampleInfo` entries (`size` left at 0 — callers fill it in once
/// they know each sample's real, possibly-reframed byte count) for a
/// WebCodecs-encoded chunk sequence in `timescale` units. `pts` and `dts`
/// are always equal: this project's WebCodecs ABR encoders never reorder
/// frames (see `mux_encoded_segment_inner`'s own `dts: None` for every
/// video PES header — TS has no B-frame reordering to express here
/// either). Each sample's duration is the gap to the next chunk's
/// timestamp, since `EncodedChunkMeta` carries no duration field directly;
/// the last sample reuses the previous gap, since there's no next
/// timestamp to measure from and an implicit zero-duration final sample
/// would undercount the fragment's real length.
...
/// Plain-Rust-error counterpart of `HlsProcessor::init_segment_video_encoded`
/// — kept separate so it (and the two `_inner` functions below it) can run
/// under a native `cargo test`: constructing a real `JsValue` needs the
/// wasm-bindgen JS-interop runtime, which only exists once actually
/// compiled to wasm32, and panics immediately otherwise — the same reason
/// `mux_segment_inner`/`mux_encoded_segment_inner` are already split out
/// this way.
fn init_segment_video_encoded_inner(width: u16, height: u16, first_keyframe_annexb: &[u8]) -> Result<Vec<u8>, String> {
    ...
    Ok(fmp4::init_segment_video(&track, width, height))
}

/// See `init_segment_video_encoded_inner`.
fn mux_video_fragment_encoded_inner(video_data: &[u8], video_meta_json: &str, sequence_number: u32) -> Result<Vec<u8>, String> {
    ...
    Ok(fmp4::fragment_video(sequence_number, &samples, &sample_data))
}

/// See `init_segment_video_encoded_inner`.
fn mux_audio_fragment_encoded_inner(
    audio_data: &[u8],
    audio_meta_json: &str,
    sequence_number: u32,
    sample_rate: u32,
    ...
    Ok(fmp4::fragment_audio(sequence_number, &samples, audio_data))
}
...
impl HlsProcessor {
    ...
    pub fn init_segment_video_encoded(&self, width: u16, height: u16, first_keyframe_annexb: &[u8]) -> Result<Box<[u8]>, JsValue> {
        init_segment_video_encoded_inner(width, height, first_keyframe_annexb)
    ...
    pub fn init_segment_audio_encoded(&self, sample_rate: u32, channels: u8) -> Box<[u8]> {
        let track = synthetic_track_data(sample_rate, vec![], [2, sample_rate_to_idx(sample_rate), channels]);
        fmp4::init_segment_audio(&track).into_boxed_slice()
    }
    ...
    pub fn mux_video_fragment_encoded(&self, video_data: &[u8], video_meta_json: &str, sequence_number: u32) -> Result<Box<[u8]>, JsValue> {
        mux_video_fragment_encoded_inner(video_data, video_meta_json, sequence_number)
            .map(Vec::into_boxed_slice)
            .map_err(|e| JsValue::from_str(&e))
    }
    ...
    pub fn mux_audio_fragment_encoded(
        ...
        mux_audio_fragment_encoded_inner(audio_data, audio_meta_json, sequence_number, sample_rate)
            .map(Vec::into_boxed_slice)
            .map_err(|e| JsValue::from_str(&e))
    }
...
mod tests {
    ...
    fn init_segment_video_encoded_builds_avcc_from_the_first_keyframe() {
        ...
        // sample entry box (see fmp4::tests::AVC1_HEADER_LEN for the same
        // constant, in a module that can't easily share it with this one).
        let avcc = find_box_in_stsd_entries(stbl, 78, b"avcC").expect("avcC");
